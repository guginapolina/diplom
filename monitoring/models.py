...
class ClientDevice(models.Model):
    ...
    target_temp = models.FloatField(default=50.0, verbose_name="Целевая температура (°C)")

    def is_metric_critical(self):
        if self.last_metric is None:
            return False
        if self.device_type == 'thermostat':
            return abs(self.last_metric - self.target_temp) > 3.0
        return self.last_metric < 100 or self.last_metric > 300
